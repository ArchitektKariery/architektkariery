...
-- Stage 6: reaching the target atomically closes funding and queues the reward.
-- Stage 7 will execute the pending reward. This migration does not activate Lucjanek.
...
begin
  ...
  if found then
    ...
    return jsonb_build_object(
      ...
      'state', v_event.state,
      'target_reached', (v_event.state in ('funded','completed'))
    );
  end if;
  ...
  v_final_state := case
    when v_new_raised >= v_event.target_qryb then 'funded'
    else 'funding'
  end;
  ...
  set raised_qryb = v_new_raised,
      ...
      state = v_final_state,
      funded_at = case
        when v_final_state = 'funded' then coalesce(funded_at, now())
        else funded_at
      end,
      closed_at = case
        when v_final_state = 'funded' then coalesce(closed_at, now())
        else closed_at
      end,
      version = version + 1
  where id = v_event.id;

  if v_final_state = 'funded' then
  ...
  return jsonb_build_object(
    ...
    'donor_count', v_new_donors,
    'state', v_final_state,
    'target_reached', (v_final_state = 'funded')
  );
end;
$$;
